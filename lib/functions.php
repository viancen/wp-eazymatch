...
function emol_get_job_search_results($reqVars, $page_slug, $searchCriteria)
{
	...
	foreach ($searchAction as $searchValue) {
		...
		} elseif (in_array($searchValue, $searchCriteria)) {
			//criteria
...
function emol_custom_title($title)
{
	...
	if (isset($emol_job_id) && strlen($emol_job_id) > 2) {
		...
		if (!is_numeric($emol_job_id)) {
			$opt = get_option('emol_job_search_page');
			$alt = get_option('emol_job_search_url');
			$base = !empty($opt) ? $opt : $alt . '/all/';
