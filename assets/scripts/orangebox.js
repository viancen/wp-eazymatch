        ...
        oB = {
            ...
            methods: {
                init: function (o) {
                    ...
                    var u = o.attr('href'), c = false, s = [0, 0], i = 0, t = "", g = false, rel = o.attr('data-ob'),
                        id, alias = false, unique = true, cap = o.attr('data-ob_caption');
                    if (!rel || rel === "") {
                        rel = o.attr('rel');
                        o.attr('data-ob', rel);
                    }
                    ...
                    if (oB.settings.checkAlias && (o.data('oB') || alias)) {
                        ...
                        if (typeof o.attr('data-ob_iframe') !== 'undefined' && o.attr('data-ob_iframe') === "true") {
                            ...
                            $.ajax({
                                url: u, dataType: 'json', success: function (data) {
                    ...
                    if (!$('#ob_content').length) {
                        ...
                        if (obj.data('oB').contentType) {
                            ...
                            var overlay = $('<div id="ob_overlay"></div>').css({
                                ...
                                }), container = $('<div id="ob_container"></div>'),
                                ob_content = $('<div id="ob_content"></div>').click(function (e) {
                                    e.stopPropagation();
                    ...
                    var href = obj.data('oB').href, title = obj.data('oB').title,
                        contentType = obj.data('oB').contentType, content, ob_caption = '', ob_caption_text = '',
                        tag = href,
                        ...
                        }), dotnav = $('<ul id="ob_dots"></ul>').click(function (e) {
                            e.stopPropagation();
                        }), ob_link;
                        ...
                        if (contentType !== "error") {
                            ...
                            if (oB.settings.addThis && obj.data('oB').share !== "false") {
                                ...
                                addthis.button('.ob_share', {
                                    ...
                                    ui_offset_left: -244,
                                    ui_offset_top: 4
                                }, {url: ob_link, title: title});
                                $('#ob_share').html('').append(shareHTML);
                            }
                            $('#ob_title').prepend(title).click(function (e) {
                                e.stopPropagation();
                            });
                            setControls();
                    ...
                    function showInline() {
                        var dim = oB.methods.getSize(obj, [0, 0]),
                            inline_content = $('<div class="inline_content"></div>'), s = obj.data('oB').size, clone,
                            copied_elem, copied_content, height;
                        if (href.match(/\?/)) {
                        ...
                        if ($(href).length && $(href).html() !== "") {
                            if (s[0] === 0) {
                                ...
                                copied_elem = $('<div id="ob_inline"></div>').css({
                                    visibility: "hidden",
                                    display: "block",
                                    position: "absolute",
                                    width: dim[1]
                            ...
                            obj.data('oB').css = dim;
                            content = $('<div id="ob_inline"></div>').css({"width": dim[1]});
                    ...
                    function showFlash() {
                        var dim = oB.methods.getSize(obj, [0, 0]);
                        obj.data('oB').css = dim;
                        content = $('<div id="ob_video">' + '<object classid="clsid:d27cdb6e-ae6d-11cf-96b8-444553540000" width="' + dim[1] + '" height="' + dim[0] + '" id="ob_flash_content" align="middle">' + '<param name="movie" value="' + href + '"/><param name="wmode" value="transparent"/><param name="allowFullScreen" value="true"/>' + '<embed name="ob_flash_content" flashVars="playerVars=autoPlay=yes" src="' + href + '" wmode="transparent" pluginspage="http://www.macromedia.com/go/getflashplayer" allowFullScreen="true" allowScriptAccess="always" width="' + dim[1] + '" height="' + dim[0] + '" type="application/x-shockwave-flash"></embed>' + '</object></div>').css({
                            "height": dim[0],
                            "width": dim[1]
                        });
                        buildWindow();
                    ...
                    function showImage() {
                        ...
                        content.load(function () {
                            var oSize = [img.height, img.width], sSize = [0, 0], running = false, dim, margin;
                            if (obj.data('oB').size) {
                            ...
                            $('#ob_content').unbind('click').click(function (e) {
                                ...
                                var fullDim = oB.methods.getSize(false, oSize, true),
                                    newDim = oB.methods.getSize(obj, [0, 0]), setDim, p = $(window).scrollTop();
                                if (p === 0) {
                                    p = $(document).scrollTop();
                                }
                                if (p === 0) {
                            ...
                            content.css({"height": dim[0], "width": dim[1]});
                            buildWindow();
                        }).error(function () {
                            throwError();
                    ...
                    if (content.href === "undefined" || content.href === "") {
                        return false;
                    }
                    var obj = $('<a href="' + content.href + '"></a>'), html;
                    if (content.title) {
                        obj.attr('title', content.title);
                    }
                    if (content.caption) {
                        obj.attr('data-ob_caption', content.caption);
                    }
                    if (content.link) {
                        obj.attr('data-ob_link', content.link);
                    }
                    if (content.linkText) {
                        obj.attr('data-ob_linkText', content.linkText);
                    }
                    if (content.linkTarget) {
                        obj.attr('data-ob_linkTarget', content.linkTarget);
                    }
                    if (content.delay) {
                        obj.attr('data-ob_delayTimer', content.delay);
                    }
                    ...
                    if (content.gallery) {
                        obj.attr('data-ob', oB.settings.searchTerm + '[' + content.gallery + ']');
                    }
                    else {
                        obj.attr('data-ob', oB.settings.searchTerm);
                    }
                    oB.methods.setupData(obj);
                    ...
                    if (oB.currentGallery[i]) {
                        ...
                        $('#ob_content').fadeOut(oB.settings.fadeTime, function () {
                            ...
                            $(this).removeClass('expanded').css({"min-height": ''}).empty();
